...
def build_merged_pk3(actordb : Array(Actor), weapon_actor_set : Set(String))
  ...
  wad_directories = Dir.glob("#{PROCESSING_DIR}/*/").map { |p| normalize_path(p) }.sort
        ...
        when "defs"
          ...
          Dir.each_child(entry_path) do |def_file|
            ...
            if decorate_file_set.includes?(def_path)
              ...
              log(1, "  defs/#{def_file} -> mm_actors/#{wad_name}/#{def_file} — found in: #{wad_name}")
            end
          end
  ...
  unless unknown_files.empty?
    ...
    unknown_files.each do |filename, wads|
      wads.each do |wad|
        if filename.starts_with?("defs/")
          bare = filename.sub("defs/", "")
          log(1, "  #{filename} -> mm_actors/#{wad}/#{bare} — found in: #{wad}")
        else
          log(1, "  #{filename} -> #{filename} (PK3 root) — found in: #{wad}")
        end
      end
    end
  end
  ...
  report_path = "#{COMPLETED_DIR}/pk3_merge_report.txt"
