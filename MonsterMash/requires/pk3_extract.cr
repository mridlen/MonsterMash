...
def copy_resource_files(src_dir : String, dest_dir : String, wad_name : String,
  ...
  Dir.each_child(src_dir) do |filename|
    ...
    # Strip .raw extension for sprites and sounds — WAD lumps have no extensions
    # and .raw breaks GZDoom's parsing:
    #   - Sprites: dual-rotation names (e.g., ISHMA1A5) won't parse with .raw
    #   - Sounds: GZDoom resolves SNDINFO lump refs by filename in sounds/ dir
    #     and does not recognize .raw as a valid audio extension in PK3s
    dest_filename = filename
    if (is_sprites_dir || strip_raw_ext) &&
       File.extname(filename).downcase == ".raw"
